...
class Node
{
    public:
        ...
        vector<nodes> expand(int elast_move_row,int elast_move_column, Board::TDCA eavailable, nodes which_node, vector<nodes> etree_makeup){
            ...
            for(int i=0;i<(int)children.size();i++){
                ...
                etemp.index = etree_makeup.size()+i; //set up the index
        ...
        vector<nodes> update(int ureward, int start_node_index,vector<nodes> utree_makeup){
            ...
            while(utree_makeup[utemp].index!=0){ // if the node is not the root
                ...
                if(utree_makeup[utemp].who_now==utree_makeup[start_node_index].who_now){ // whether the node has the same player as the stimulated node
                    utree_makeup[utemp].score+=ureward; // add reward to score
                }
                else{
                    utree_makeup[utemp].score-=ureward; // subtract reward from score
                }
            ...
            utree_makeup[utemp].visit_time++; //for root node
            if(utree_makeup[utemp].who_now==utree_makeup[start_node_index].who_now){ // whether the node has the same player as the stimulated node
                utree_makeup[utemp].score+=ureward; // add reward to score
            }
            else{
                utree_makeup[utemp].score-=ureward; // reduct reward from score
            }
        ...
        nodes selection(nodes start_select,vector<nodes> stree_makeup){
            ...
            for(int i=0;i<(int)start_select.sons.size();i++){
                visit_count += stree_makeup[start_select.sons[i]].visit_time;
            }
            for(int i=0;i<(int)start_select.sons.size();i++){
                if(stree_makeup[start_select.sons[i]].visit_time == 0){
                    cout<<"picked the "<<start_select.sons[i]<<" component of the tree"<<endl;
        ...
        Board::coordinate MCTS(int mlast_move_row, int mlast_move_column, Board::TDCA main_board, Board::TDCA main_available, char pawn){
            ...
            nodes root; //the new root is not initialize properly
            root.index=0;
            root.mother=-1;
            root.who_now = pawn;
            root.final_value = 0;
            ...
            while(running_times>0){
                ...
                cout<<"endfor"<<endl;
                cout<<"mlast_move_row=" << mlast_move_row;
                cout<<"mlast_move_column="<< mlast_move_column;
                if(tree_makeup.size()==1){ cout<<"begin expand"<<endl;
                    temp_tree_size = (int)tree_makeup.size();
                    tree_makeup = expand(mlast_move_row,mlast_move_column, temp_available, tree_makeup[0], tree_makeup); cout<<"end expand"<<endl; //set up basic tow depth tree
                }
                for(int i=temp_tree_size; i<(int)tree_makeup.size();i++){
                    tree_makeup[0].sons.push_back(i);
                }
                nodes leave = tree_makeup[0]; cout<<"to while"<<endl;
                for(int i=0;i<(int)tree_makeup.size();i++){
                    cout<<"The visit time of the "<<i<<" component of the tree is "<<tree_makeup[i].visit_time<<endl;
                }
                while(leave.sons.size()!=0){cout<<"begin select"<<endl; //keep selecting until reach the leave
                    leave = selection(leave, tree_makeup); cout<<"end select"<<endl;
                }cout<<"end while"<<endl;
                cout<<"leave.action.xy[0]="<<leave.action.xy[0]<<endl;
                cout<<"leave.action.xy[1]="<<leave.action.xy[1]<<endl;
                if(leave.visit_time==0){ cout<<"visit time 0"<<endl; //if leave hasn't been visited
                    tree_makeup = update(stimulate(leave.action.xy[0],leave.action.xy[1],leave.who_now,temp_available,temp_board),leave.index, tree_makeup); cout<<"end update for 0"<<endl;//do stimulation
                }
                else{cout<<"visit time >0"<<endl; //if leave was visited
                    temp_tree_size = (int)tree_makeup.size();
                    tree_makeup = expand(mlast_move_row,mlast_move_column, temp_available, tree_makeup[0], tree_makeup); //expand the leave
                    for(int i=temp_tree_size; i<(int)tree_makeup.size();i++){
                        tree_makeup[0].sons.push_back(i);
                    }
                    leave = selection(leave, tree_makeup); //select an expanded leave
                    tree_makeup = update(stimulate(leave.action.xy[0],leave.action.xy[1],leave.who_now,temp_available,temp_board),leave.index, tree_makeup); cout<<"end update"<<endl;//do stimulation
                }
                running_times--;
                cout<<"end big while once"<<endl;
            }
